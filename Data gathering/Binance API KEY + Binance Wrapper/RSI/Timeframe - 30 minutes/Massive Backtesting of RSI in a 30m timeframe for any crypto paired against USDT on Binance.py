...
not_wanted_coins = ["TUSD", "BUSD", "USDC", "USDP", "USTC", "DAI", "AUD", "BIDR", "BRL", "BCHSV", "BULL", "BEAR", "USDS", "ERD", "UST",
                    "EUR", "GBP", "RUB", "TRY", "UAH", "IDRT", "NGN", "UP", "DOWN", "BCHABC", "SUSD", "BCC", "PAX", "NANO", "VEN",
                    "NPXS", "STORM", "HC", "MCO", "XZC", "LEND", "BKRW", "BZRX", "KEEP", "ANY", "BTT", "STRAT", "NU", "RGT", "NBT"]
#Delete not wanted coins
for ticket in usdt_trading_pairs[:]:
    for coin in not_wanted_coins:
        if coin in ticket[:len(coin)]:
            usdt_trading_pairs.remove(ticket)
            
        elif len(ticket) == 11 and coin in ticket[3:-4]: #delete bull and bear related tokens
            usdt_trading_pairs.remove(ticket)
            
        elif "UP" in ticket[-6:] and coin == "UP": #delete UP related tokens
            usdt_trading_pairs.remove(ticket)
            
        elif "DOWN" in ticket[-8:] and coin == "DOWN": #delete DOWN related tokens
            usdt_trading_pairs.remove(ticket)

#Now do the big process
start_date_unix = "1652349600000"
end_date_unix = str(int(start_date_unix[:-3])+1*(30*60*1025)-1)+"999"
profitable_trading_pairs = []

for trading_pair in usdt_trading_pairs:
    print(f'Analyzing trading pair No. {usdt_trading_pairs.index(trading_pair)} out of {len(usdt_trading_pairs)}')
    ...
    if len(klines) == 0:
        print(f"This trading pair {trading_pair} has no data!")
        deprecated_trading_pairs.append(trading_pair)
        ...
        if max(np.append(100, 100*np.cumprod(1 + RSI_gross_return))) > max(np.append(100, 100*np.cumprod(1 + df_trading_pair["Return"]))) and max(np.append(100, 100*np.cumprod(1 + RSI_gross_return))) >= 130 :
